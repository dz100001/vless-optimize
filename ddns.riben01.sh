...
if [ -z "$MY_ROOT_PASSWORD" ] || [ -z "$MY_CF_API_KEY" ]; then
    echo "错误：未提供安全信息 (API, MY_ROOT_PASSWORD)！"
    echo "脚本已终止，以确保系统安全。"
    exit 1
fi

# -----------------------------------------------------------------
# 1. 配置信息 (从命令行接收数据，不硬编码)
# -----------------------------------------------------------------
...
CF_RECORD="${MY_CF_RECORD:-aliyun.riben01.c3c3.top}"

# -----------------------------------------------------------------
# 2. 安装基础工具 (CURL, WGET) & 更新系统
# -----------------------------------------------------------------
echo "==> 正在安装 curl、wget 并更新系统软件..."
